...
export default function Contact() {
  ...
  const handleSubmit = async (values: ContactFormValues) => {
    try {
      ...
      // 1. Lưu vào Firebase như cũ
      await addDoc(collection(db, "contacts"), {
        ...values,
        createdAt: serverTimestamp(),
      });

      // 2. Gửi Telegram notification — fire-and-forget, không block
      fetch("/api/contact", {
        method: "POST",
        ...
        body: JSON.stringify({
          name: values.name,
          email: values.email,
          // Ghép subject vào message để Telegram hiện đủ context
          message: values.subject
            ? `[${values.subject}] ${values.message}`
            : values.message,
        }),
      }).catch(() => {}); // silent — không làm hỏng UX nếu Telegram lỗi
          ...
          <a href="mailto:dung.dev.web@gmail.com" className="block">
            <Card className="p-6 flex items-center gap-4 hover:shadow-lg transition-shadow">
              ...
              <div>
                ...
                <p className="font-semibold">dung.dev.web@gmail.com</p>
              </div>
            </Card>
          </a>
