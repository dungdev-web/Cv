...
function SkillCard({ skill, index }: { skill: typeof skills[0]; index: number }) {
  const tilt = useTilt();
  return (
    <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.08 }}>
      <div ref={tilt.ref} onMouseMove={tilt.onMove} onMouseLeave={tilt.onLeave} style={{ transition: "transform 0.18s ease-out", transformStyle: "preserve-3d" }}>
        <Card className="p-5 hover:shadow-xl transition-shadow border-2 hover:border-primary/30 group relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-3" style={{ transform: "translateZ(16px)" }}>
            ...
            <motion.span className="text-sm font-bold text-primary tabular-nums" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 + index * 0.08 }}>
              {skill.level}%
            </motion.span>
          </div>
          <div className="h-2.5 w-full rounded-full bg-secondary overflow-hidden" style={{ transform: "translateZ(8px)" }}>
            <motion.div
            ...
            >
              <div className="absolute inset-0 blur-sm opacity-60" style={{ background: "inherit" }} />
            </motion.div>
          </div>
        </Card>
      </div>
...
// ── Expertise card ─────────────────────────────────────────────────────────────
function ExpertiseCard({ iconComp: Icon, title, description, index }: { iconComp: React.ElementType; title: string; description: string; index: number }) {
  const tilt = useTilt();
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 + index * 0.07 }}>
      <div ref={tilt.ref} onMouseMove={tilt.onMove} onMouseLeave={tilt.onLeave} style={{ transition: "transform 0.18s ease-out", transformStyle: "preserve-3d", height: "100%" }}>
...
function SpiderChart({ title }: { title: string }) {
  const size = 300;
  const cx = size / 2; const cy = size / 2; const r = 108;
  const step = 360 / spiderData.length;
  const pt = (angle: number, dist: number) => {
    const rad = (angle - 90) * (Math.PI / 180);
    return { x: cx + dist * Math.cos(rad), y: cy + dist * Math.sin(rad) };
  };
  const dataPoints = spiderData.map((d, i) => { const p = pt(i * step, (d.value / 100) * r); return `${p.x},${p.y}`; }).join(" ");
  const gridPolygon = (pct: number) => spiderData.map((_, i) => { const p = pt(i * step, (pct / 100) * r); return `${p.x},${p.y}`; }).join(" ");
          ...
          <div className="flex justify-center">
            <svg width="100%" viewBox={`0 0 ${size} ${size}`} style={{ maxWidth: size }}>
              {[20, 40, 60, 80, 100].map((lvl) => (
                <polygon key={lvl} points={gridPolygon(lvl)} fill="none" stroke="currentColor" strokeOpacity={0.08} strokeWidth={1} />
              ))}
              {spiderData.map((_, i) => {
              ...
              {spiderData.map((d, i) => {
                ...
                return <text key={i} x={p.x} y={p.y} textAnchor="middle" dominantBaseline="middle" fontSize={11} fontWeight={600} fill="currentColor" opacity={0.7}>{d.label}</text>;
              })}
            </svg>
          </div>
        </div>
...
export default function Skills() {
  ...
  return (
    <section id="skills" className="relative container mx-auto px-4 py-20 scroll-mt-20 overflow-hidden">
      <motion.div className="absolute top-20 left-10 w-72 h-72 bg-purple-500/5 rounded-full blur-3xl -z-10" animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.5, 0.3] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }} />
      <motion.div className="absolute bottom-20 right-10 w-80 h-80 bg-cyan-500/5 rounded-full blur-3xl -z-10" animate={{ scale: [1.2, 1, 1.2], opacity: [0.3, 0.5, 0.3] }} transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }} />

      {/* Title */}
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
        <Badge className="mb-4 px-4 py-2"><Wrench className="w-4 h-4 mr-2" />{t.skills.badge}</Badge>
        <h2 className="text-4xl md:text-5xl font-bold mb-4">
          {t.skills.heading1} <span className="text-primary">{t.skills.heading2}</span>{t.skills.heading3}
        </h2>
      ...
      <div className="grid lg:grid-cols-5 gap-8 mb-8">
        <div className="lg:col-span-3 space-y-4">
          <motion.h3 initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="text-2xl font-bold mb-6 flex items-center gap-2">
            <Code2 className="w-6 h-6 text-primary" />{t.skills.coreTitle}
          </motion.h3>
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
            <Card className="p-6 border-2 hover:border-primary/30 transition-all text-center bg-gradient-to-br from-primary/5 to-transparent">
              ...
              <div className="text-sm text-muted-foreground font-medium">{t.skills.techMastered}</div>
            </Card>
          </motion.div>
            ...
            <Card className="p-6 border-2 hover:border-primary/30 transition-all relative overflow-hidden group">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-5">
                  <div className="p-2.5 rounded-xl bg-primary/10"><Wrench className="w-5 h-5 text-primary" /></div>
                  <h3 className="text-xl font-bold">{t.skills.toolsTitle}</h3>
                </div>
                <div className="flex flex-wrap gap-2.5">
                  {tools.map((tool, i) => (
                    <motion.div key={tool.name} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} whileHover={{ scale: 1.1, y: -2 }}>
                      <Badge variant="secondary" className="px-3 py-2 text-sm font-medium hover:bg-primary/10 transition-colors cursor-pointer">
                        <tool.icon className="w-3.5 h-3.5 mr-1.5" />{tool.name}
                      </Badge>
                    </motion.div>
                  ))}
                </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mb-6">
        <h3 className="text-2xl font-bold flex items-center gap-2 mb-6">
          <Zap className="w-6 h-6 text-primary" />{t.skills.expertiseTitle}
        </h3>
      </motion.div>
